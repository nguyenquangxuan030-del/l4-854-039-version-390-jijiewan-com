...
function initVideo(frame) {
  ...
  var sourceIndex = Number(video.dataset.sourceIndex || 0);
  var source = sourceByIndex(sourceIndex);
  var hls = null;
  ...
  video.addEventListener("pause", function () {
    if (coverButton && video.currentTime === 0) {
      coverButton.classList.remove("is-hidden");
    }
  });
...
document.addEventListener("DOMContentLoaded", function () {
  document.querySelectorAll("[data-player]").forEach(initVideo);
});
